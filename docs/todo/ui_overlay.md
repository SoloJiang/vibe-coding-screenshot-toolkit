...
- ✅ **交互流程优化（v0.1.3）**：修改为拖动结束后需按 Enter 确认，避免误操作
- ✅ **竖屏窗口尺寸错误（v0.1.3）**：重构为使用 winit 单一信源，创建窗口时指定尺寸和位置
- ✅ **部分显示器无覆盖窗口（v0.1.3）**：移除手动计算的 monitor_layouts，直接使用 winit 的 available_monitors()
